...
public class CardHolderValidatorTest {

    @Test
    public void cardHolderIsNullTest() {
        CardInfo card = new CardInfo();
        card.setCardHolder(null);
        
        CardHolderValidator validator = new CardHolderValidator();
        String msg = "";
        try {
            validator.validate(card);
            fail("ApplicationException was expected. Cardholder field cannot be null");
        }
        catch (ApplicationException e) {
            msg = e.getMessage();
        }
        
        Assert.assertTrue(msg.toLowerCase().contains("missing: cardholder"));
    }
